...
class MoviesView(Resource):
    def get(self):
        ...
        if request.args.get('director_id') is not None:
            res = res.filter(Movie.director_id == request.args.get('director_id'))
        if request.args.get('genre_id') is not None:
            res = res.filter(Movie.genre_id == request.args.get('genre_id'))
        result = res.all()
